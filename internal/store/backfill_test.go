...
type issueRow struct {
	typ, summary, status, statusCategory, size, sprint, assignee string
}

func readIssue(t *testing.T, st *Store, key string) issueRow {
	t.Helper()
	var r issueRow
	var size, sprint, assignee any
	err := st.db.QueryRow(
		`SELECT type, summary, status, status_category, size, sprint, assignee FROM issue WHERE key=?`, key,
	).Scan(&r.typ, &r.summary, &r.status, &r.statusCategory, &size, &sprint, &assignee)
	if err != nil {
